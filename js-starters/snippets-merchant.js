;(function () {
  ...
  const sellTrash = () =>
    character.items.forEach((o, i) => {
      if (isTrash(o)) sell(i, o.q || 1)
    })

  const bankRetrieveTypes = types =>
    arrayOf(types).forEach(type => {
      bankPackKeys.forEach(x =>
        bankPack(x).forEach((o, slot) => {
  ...
  const bankRetrieve = ({ name, type, level }) =>
    bankPackKeys.forEach(packKey => {
  ...
  const bankRetrieveCompoundables = () =>
    bankPackKeys.forEach(x =>
      bankPack(x).forEach(o => {
